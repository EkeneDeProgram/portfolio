...
export default function BlogPostCard({ post }: BlogPostCardProps) {
  ...
  const imageSrc =
    post.featuredImage?.url?.startsWith("http")
      ...
      : post.featuredImage?.url
      ? `${CMS_URL}${post.featuredImage.url}`
      : "/images/news-placeholder.jpg"; // fallback
  ...
  return (
    <li className="group flex flex-col sm:flex-row sm:items-start gap-4 sm:gap-6 pb-8 border-b last:border-b-0">
      ...
      <div className="flex-1">
        ...
        <Link
          href={`/blog/${post.slug}`}
          className="block mt-1 text-lg sm:text-xl lg:text-2xl font-bold text-gray-900 hover:text-blue-600"
        >
          {post.title}
        </Link>

        <p className="mt-2 text-gray-800 text-sm sm:text-base line-clamp-3">
          {post.excerpt}
        </p>

        <time className="block mt-4 text-xs sm:text-sm text-gray-600">
          {new Date(post.createdAt).toLocaleDateString()}
        </time>
      </div>
    </li>
